...
const DeviceNotSendingData = (props) => {
    ...
    const getCurrentLocation = async (id) => {
        try{
            ...
            return {location: response.data[0]["address"], timestamp: convertEpochToDateAndTime(response.data[0]["gps_ts"]), speed: response.data[0]["gps_speedKph"], epochTime: response.data[0]["gps_ts"]}
        } catch{
            return {location: "", timestamp: "", speed: "", epochTime:""}
        }
      }
    ...
    const searchOldFun = async () => {
        ...
        try {
            ...
            const devicesData = data?.result?.devicelist
            console.log(devicesData, "devicesData")
            const devicesDataWithLocationall = []
            if(devicesData?.length > 0){
                for (let i = 0; i < devicesData.length; i++) {
                    const getLocation = await getCurrentLocation(devicesData[i]["intuchEntityId"])
                    devicesDataWithLocationall.push({...devicesData[i], location: getLocation["location"], timestamp: getLocation["timestamp"], speed: getLocation["speed"], epochLive: getLocation["epochTime"]})
                }
            }
            const currentEpoch = Math.floor(Date.now() / 1000)
            const devicesDataWithLocation = devicesDataWithLocationall.filter(item=>(currentEpoch-item?.epochLive) > 24*60*60)
            ...
            {/* 👉 Listtable Components 👈 */}
            {
                ...
                <>
                    ...
                    <ListTable
                        ...
                        heading={"Device Not Sending Data"}
                    />
                </>
...
export default DeviceNotSendingData