...
import { getInfoReactivateLive, searchEMEIDevice } from "../../apis/deviceInventory";
import Swal from "sweetalert2";
import { useLocation } from "react-router-dom";
import './activateDevice.css'
import { LoadingWidget } from "../../components/loading";
import { InfoDataVahanComponent } from "./components/infoDataVahanComponent";
import { ReActivationComponentLive } from "./components/reactivationComponentLive";

export const ReactivationVahan = () => {
  ...
  const getDeviceInfo = async () => {
    const result_vahan_esim = await getInfoReactivateLive(String(id), String(IMEI));
  ...
  return (
    <>
      ...
      {data && data["status"] === 1 && !toggle && <InfoDataVahanComponent data = {data} getInfo= {getDeviceInfo} setLoading={setLoading}/>}
      {data && data["status"] !== 1 && <p style={{color:"red", textAlign:"center", marginTop:"10px"}}><b>Reactivation allowed for active devices only!!</b></p>}
      
      {loading ? <LoadingWidget/> : <>{toggle && infoData && <ReActivationComponentLive infoData={infoData}/>}</>}
