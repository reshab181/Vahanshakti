...
import { assigntorfcuser, imeiToList } from "../../apis/deviceInventory";
...
import { getUsersInRFC } from "../../apis/rfcMaster";

export const DeviceInventoryListRfcUserAssign = () => {
  ...
  const [rfcUserList, setRfcUserList] = useState([]) // To store RFC user list
  const [rfcUser, setRfcUser] = useState(null) // To store selected RFC user
  ...
  console.log(rfcUserList, "rfcUserList")

  const fetchRFCUsersList = async () => {
  const response = await getUsersInRFC()
  console.log(response, "fetchRFCUsersList")

  if(response?.status){
    setRfcUserList(response?.result)
  }
}
...
useEffect(() => {
  ...
  const assingFun = async (obj, type = '') => {
    ...
    switch (type) {
      ...
      case 'bulk': {
        ...
        body = bulkData?.map((item) => {
          return { ...item, userId: parseInt(rfcUser) };
        });
    ...
    const apiHitting = await assigntorfcuser(body)
...

                {
                  ...
                  <>
                    <div className="cell_action">
                      <select name="" id="" disabled={loader} onChange={(e) => setRfcUser(e.target.value)} value={rfcUser}>
                        <option value="">Select</option>
                        {
                          Array.isArray(rfcUserList) &&
                          rfcUserList?.map((elem, index) =>
                      ...
                      {!loader ? <>
                        {rfcUser != -1 && <attr title="Confirm" className="cell_check" onClick={() => assingFun(cell?.row?.original)}><ImCheckmark size={18} /></attr>}
    ...
    <>
      ...
      <div className="alarm_header">
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '10px' }}>
          ...
          {userType == 'RFC' && <div className="bulk_assign_button">
            {!bulkStatus ?
              ...
              <>
                <div className="cell_action">
                  ...
                  {!loader ? <>
                    {rfcUser != -1 && <attr title="Confirm" className="cell_check" onClick={() => assingFun(null, 'bulk')}><ImCheckmark size={18} /></attr>}
      ...
      {/* Tables */}
      {
        (!excelLoader && !excelView) ?
          <>{
            ...
            userType === "RFC" &&
            <ListTableConnect
              api={`${BaseURL}/DeviceInventory/GetDeviceAllListRfc?eid=${eid}&fromDt=${fromDate}&toDt=${uptoDate}&status=${0}&rfcassign=${0}&`}
              method="GET"
              columns={column}
