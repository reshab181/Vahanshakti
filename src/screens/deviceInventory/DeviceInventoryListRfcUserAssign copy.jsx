...
import { assigntorfc, assigntorfcuser, imeiToList } from "../../apis/deviceInventory";
...
export const DeviceInventoryListRfcUserAssign = () => {
  ...
  const [rfcUser, setRfcUser] = useState([]) // To store RFC user list
    ...
    switch (type) {
      ...
      case 'bulk': {
        ...
        body = bulkData?.map((item) => {
          return { ...item, userId: parseInt(entityId) };
        });
      ...
      {/* Tables */}
      {
        (!excelLoader && !excelView) ?
          <>{
            ...
            userType === "RFC" &&
            <ListTableConnect
              api={`${BaseURL}/DeviceInventory/GetDeviceAllListRfc?eid=${eid}&fromDt=${fromDate}&toDt=${uptoDate}&status=${-1}&rfcassign=${-1}&`}
              method="GET"
              columns={column}
