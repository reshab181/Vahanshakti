...
import { assigntodstuser, imeiToList } from "../../apis/deviceInventory";
...
import { getUsersInDST } from "../../apis/distributor";
...
export const DeviceInventoryListDSTUserAssign = () => {
  ...
  const [dstUserList, seDSTUserList] = useState([]) // To store DST user list
  const [dstUser, setDSTUser] = useState(null) // To store selected DST user
  ...
  console.log(dstUserList, "dstUserList")

  const fetchDSTUsersList = async () => {
  const response = await getUsersInDST()
  console.log(response, "fetchDSTUsersList")

  if(response?.status){
    seDSTUserList(response?.result)
  }
}
...
useEffect(() => {
  ...
  const assingFun = async (obj, type = '') => {
    ...
    switch (type) {
      ...
      case 'bulk': {
        ...
        body = bulkData?.map((item) => {
          return { ...item, userId: parseInt(dstUser) };
        });
    ...
    const apiHitting = await assigntodstuser(body)
  ...
  const handleBulkStatus = (status) => {
    ...
    if (status) {
      ...
      let tempData = dataList?.filter(item => (item?.ctStatus == 1 && item?.rfcUserId == 0))?.map((elem) => {
        return {
          id: String(elem?.id),
          imeiNo: elem?.imeiNo,
        }
...

                {
                  ...
                  <>
                    <div className="cell_action">
                      <select name="" id="" disabled={loader} onChange={(e) => setDSTUser(e.target.value)} value={dstUser}>
                        <option value="">Select</option>
                        {
                          Array.isArray(dstUserList) &&
                          dstUserList?.map((elem, index) =>
                            <option key={index} value={elem?.userid}>{elem?.username}</option>
                          )
                        }
                      </select>

                      {!loader ? <>
                        {dstUser != -1 && <attr title="Confirm" className="cell_check" onClick={() => assingFun(cell?.row?.original)}><ImCheckmark size={18} /></attr>}
    ...
    <>
      ...
      <div className="alarm_header">
        ...
        <div className='heading_container'>
          Distributor User Assign Inventory List <i style={{ fontWeight: '600', fontSize: '14px' }}>{excelView && "(Showing Excel Data List)"}</i>
        </div>
                ...
                <div className="cell_action">
                  <select name="" id="" disabled={loader} onChange={(e) => setDSTUser(e.target.value)} value={dstUser}>
                    <option value={-1} disabled>Select</option>
                    {
                      Array.isArray(dstUserList) &&
                      dstUserList?.map((elem, index) =>
                        <option key={elem.userid} value={elem.userid}>{elem?.username}</option>
                      )
                    }
                  </select>

                  {!loader ? <>
                    {dstUser != -1 && <attr title="Confirm" className="cell_check" onClick={() => assingFun(null, 'bulk')}><ImCheckmark size={18} /></attr>}
      ...
      {/* Tables */}
      {
        (!excelLoader && !excelView) ?
          <>{
            ...
            userType === "DST" &&
            <ListTableConnect
              api={`${BaseURL}/DeviceInventory/GetDeviceAllListassignDst?fromDt=${fromDate}&toDt=${uptoDate}&status=${-1}&assign=${0}&`}
              method="GET"
              columns={column}
